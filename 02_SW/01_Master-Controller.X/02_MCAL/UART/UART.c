...
/*                               Local defines                                */
/*----------------------------------------------------------------------------*/
#define FREQ_CLK                (64000000u)
#define RESET_VALUE             (0x00u)
#define LSB_MASK                (0xF0u)
...
void UART1_vInit(void)
{
    ...
    RC7PPS = 0x17;              //  UART1 => set RX on PORTC pin 7 
...
{
    while(1 == MASK_8BIT_GET_BIT(PIR3,3))
    {
...
void UART1_vBaudCalculator( BaudGeneratorSpeed GeneratorSpeed, uint32_t DesiredBaud)
{
    ...
    else if(HIGH_SPEED == GeneratorSpeed)
    {
        ...
        U1BRG = (FREQ_CLK / (4 * DesiredBaud)) - 1; // BaudRate formula for HighSpeed MODE
    }
    else if(NORMAL_SPEED == GeneratorSpeed)
    {
        ...
        U1BRG = (FREQ_CLK / (16 * DesiredBaud)) - 1; // BaudRate formula for NormalSpeed MODE
    }
    else
    {
